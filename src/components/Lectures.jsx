    ...
    return (
        ...
        <motion.div style={{ opacity: containerOpacity, zIndex: 70 }}
            ...
            <motion.div style={{ y: containerY }} className="w-full h-full">
                ...
                <div className="hidden md:flex w-full h-full items-center justify-center px-6 lg:px-10 xl:px-16 pointer-events-auto">
                    <div className="w-full max-w-6xl grid grid-cols-[1.15fr_1fr] gap-5 lg:gap-7 items-stretch"
                        style={{ height: 'clamp(440px, 72vh, 580px)' }}>

                        {/* ── Levá karta — informační ── */}
                        <div className="bg-white/94 backdrop-blur-2xl border border-white/70 shadow-2xl rounded-[2rem] overflow-hidden flex flex-col">
                            <div className="px-7 lg:px-9 pt-7 lg:pt-8 pb-4 border-b border-slate-100/80">
                                <p className="text-gold-600 font-sans uppercase tracking-[0.3em] text-[10px] font-bold mb-3">08 — Přednášky (7200 m)</p>
                                <h2 className="font-serif text-3xl lg:text-4xl text-slate-900 leading-tight mb-3">
                                    Příběhy z hor,<br/>cest i <span className="italic text-amber-800/65">návratů.</span>
                                </h2>
                                <p className="font-sans text-slate-600 text-sm leading-relaxed max-w-md">
                                    Veřejné besedy, firemní akce, školy i festivaly. Vyprávění o Nepálu, vysokých horách, zdraví a návratech — i o obyčejných věcech, které člověku cesty postupně dají.
                                </p>
                            </div>

                            <div className="flex-1 px-7 lg:px-9 py-4 overflow-y-auto overscroll-contain flex flex-col gap-4" data-lenis-prevent>
                                {/* Pro koho */}
                                <div>
                                    <p className="text-[9px] font-bold uppercase tracking-[0.35em] text-slate-400 mb-2.5">Pro koho</p>
                                    <div className="grid grid-cols-2 gap-1.5">
                                        {AUDIENCES.map((a, i) => (
                                            <div key={i} className="flex items-center gap-2 bg-slate-50 border border-slate-100 rounded-xl px-3 py-2">
                                                <span className="text-gold-500 shrink-0">{a.icon}</span>
                                                <span className="text-slate-700 text-xs font-semibold leading-tight">{a.label}</span>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                                ...
                                {/* Témata */}
                                <div>
                                    <p className="text-[9px] font-bold uppercase tracking-[0.35em] text-slate-400 mb-2.5">Témata přednášek</p>
                                    <div className="flex flex-wrap gap-1.5">
                                        {TOPICS.map(t => (
                                            <button key={t.id} onClick={() => setShowTopics(true)}
                                                className="group flex items-center gap-1.5 bg-white border border-slate-200 hover:border-gold-300 hover:bg-gold-50 rounded-full px-3 py-1.5 transition-all duration-200">
                                                <span className="text-xs font-semibold text-slate-700 group-hover:text-gold-700">{t.title}</span>
                                            </button>
                                        ))}
                                    </div>
                                </div>
                                ...
                                <a href="https://www.honzatravnicek.cz/prednaska/" target="_blank" rel="noopener noreferrer"
                                    className="inline-flex items-center gap-2 text-gold-600 hover:text-gold-700 text-xs font-bold uppercase tracking-widest transition-colors w-fit">
                                    Nabídka & anotace <ExternalLink className="w-3 h-3" />
                                </a>
                            </div>

                            {/* CTA */}
                            <div className="px-7 lg:px-9 py-4 border-t border-slate-100 flex gap-2 shrink-0">
                                <button onClick={() => setBookingOpen(true)}
                                    className="group flex-1 flex items-center justify-center gap-2 bg-slate-900 hover:bg-gold-600 text-white text-xs font-bold uppercase tracking-widest py-3.5 rounded-xl transition-all shadow-lg">
                                    Booking & Kontakt <ArrowRight className="w-3.5 h-3.5 group-hover:translate-x-0.5 transition-transform" />
                                </button>
                                <button onClick={() => setShowTopics(true)}
                                    className="px-4 py-3.5 bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-600 text-xs font-bold uppercase tracking-widest rounded-xl transition-all">
                                    Více témat
                                </button>
                            </div>
                        </div>

                        {/* ── Pravá karta — akce & kalendář ── */}
                        <div className="bg-slate-900/80 backdrop-blur-2xl border border-white/10 shadow-2xl rounded-[2rem] overflow-hidden flex flex-col">
                            <div className="px-6 lg:px-8 pt-7 pb-4 border-b border-white/[0.07]">
                                <p className="text-gold-400 font-mono text-[9px] uppercase tracking-[0.4em] font-bold mb-2">Akce & turné</p>
                                <h3 className="font-serif text-2xl lg:text-3xl text-white leading-tight">
                                    Nadcházející<br/><span className="italic text-slate-400">přednášky.</span>
                                </h3>
                            </div>

                            <div className="flex-1 px-6 lg:px-8 py-5 flex flex-col gap-4 overflow-y-auto" data-lenis-prevent>
                                {/* Projekty karty */}
                                {EVENTS_DETAIL.map(ev => (
                                    <button key={ev.id} onClick={() => setSelectedEvent(ev)}
                                        className="group text-left flex gap-3 bg-white/[0.06] hover:bg-white/[0.10] border border-white/[0.08] hover:border-gold-500/30 rounded-2xl p-4 transition-all duration-200">
                                        <div className="w-16 h-14 rounded-xl overflow-hidden shrink-0">
                                            <img loading="lazy" src={ev.image} alt={ev.city} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-400" />
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <p className="text-gold-400 font-mono text-[9px] uppercase tracking-widest mb-0.5">{ev.date}</p>
                                            <p className="text-white text-sm font-semibold leading-tight truncate">{ev.city}</p>
                                            <p className="text-slate-500 text-[10px] mt-0.5 leading-tight line-clamp-2">{ev.venue}</p>
                                        </div>
                                        <ArrowRight className="w-4 h-4 text-slate-600 group-hover:text-gold-400 shrink-0 mt-1 transition-colors" />
                                    </button>
                                ))}
                                ...
                                <div className="rounded-2xl border border-dashed border-white/15 p-4 text-center">
                                    <p className="text-slate-500 text-xs mb-3 leading-relaxed">
                                        Aktuální termíny přednášek se průběžně doplňují v kalendáři.
                                    </p>
                                    <button onClick={() => setCalendarOpen(true)}
                                        className="inline-flex items-center gap-2 text-gold-400 hover:text-gold-300 text-xs font-bold uppercase tracking-widest transition-colors">
                                        <Calendar className="w-3.5 h-3.5" /> Otevřít kalendář akcí
                                    </button>
                                </div>
                            </div>
                        </div>

...
const Lectures = ({ scrollProgress }) => {
    ...
    const containerOpacity = useTransform(scrollProgress, [0.62, 0.65, 0.68, 0.71], [0, 1, 1, 0]);
    const containerY = useTransform(scrollProgress, [0.62, 0.65, 0.68, 0.71], ["-120%", "0%", "0%", "120%"]);

    // Lightens the background as it exits
    const lightenOpacity = useTransform(scrollProgress, [0.67, 0.71], [0, 1]);

    const lectCarouselRef = useRef(null);
    const lectCarouselProgress = useTransform(scrollProgress, [0.65, 0.68], [0, 1]);
    useEffect(() => {

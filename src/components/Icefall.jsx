...
const Icefall = ({ scrollProgress }) => {
    ...
    useScrollLock(selectedFlag || isInfoOpen);
        ...
        <>
            ...
            >
                <div className="w-full h-full relative">
                    {/* ── Header text ── */}
                    <div className="absolute top-[5%] md:top-[8%] left-0 right-0 text-center z-20 px-5">
                        ...
                        <p className="font-sans text-slate-600 text-xs md:text-sm leading-relaxed mt-3 max-w-sm md:max-w-lg mx-auto">
                            Máme rádi dlouhodobé vztahy a značky, které si na nic nehrají — stejně jako my. Většinu z nich sami používáme v horách i v běžném životě.
                        </p>
                        <button
                    ...
                    <div className="absolute bottom-[18%] md:bottom-[16%] left-0 right-0 text-center z-20 pointer-events-none flex flex-col items-center gap-2 px-4">
                        ...
                        {/* Past partners — solid white pill row, always visible */}
                        <div className="flex flex-col items-center gap-1.5 pointer-events-auto">
                            <span className="text-[9px] font-sans font-bold uppercase tracking-[0.25em] text-slate-400">Dříve také:</span>
                            <div className="flex flex-wrap gap-1.5 justify-center">
                                {PAST_PARTNER_NAMES.map((name) => (
                                    <span
                                        key={name}
                                        className="text-[11px] font-sans font-medium text-slate-600 px-3 py-1 rounded-full bg-white/95 shadow-sm border border-slate-200"
                                    >
                                        {name}
                                    </span>
                                ))}
                ...
                {isInfoOpen && (
                    ...
                    >
                        <motion.div
                            ...
                            className="bg-ivory w-full max-w-2xl max-h-[92vh] rounded-3xl overflow-hidden shadow-2xl border border-white/10 flex flex-col"
                        >
                            {/* Hero */}
                            <div className="relative h-32 md:h-40 shrink-0 overflow-hidden">
                                ...
                                <div className="absolute bottom-4 left-6">
                                    <span className="text-gold-400 font-mono text-[10px] uppercase tracking-[0.4em] font-bold block mb-1">Spolupráce</span>
                                    <h2 className="font-serif text-xl md:text-2xl text-white leading-none">Partneři na cestě.</h2>
                                </div>
                            </div>

                            {/* Body */}
                            <div className="flex-1 overflow-y-auto overscroll-contain custom-scrollbar px-6 md:px-9 py-6 md:py-8" data-lenis-prevent>
                                ...
                                <p className="font-sans text-slate-600 leading-relaxed text-sm mb-6">
                                    Většinu partnerů používáme v horách, na cestách i v běžném životě mnoho let. Ať už jde o
                                    vybavení do vysokých hor, jídlo na expedice, oblečení, outdoorové zázemí nebo projekty
                                ...
                                <div className="flex flex-wrap gap-2 mb-6">
                                    {FLAGS_DISPLAY.map((f) => (
                                        <button
                                            key={f.id}
                                            onClick={() => { setIsInfoOpen(false); setTimeout(() => setSelectedFlag(f), 50); }}
                                            className="group flex items-center gap-2 px-3.5 py-2 bg-white rounded-xl border border-slate-200 shadow-sm hover:border-gold-400 hover:shadow-md transition-all duration-200 text-left"
                                        >
                                            <span className="font-sans text-slate-800 text-xs font-bold">{f.name}</span>
                                            {f.discount && (
                                                <span className="text-[9px] font-bold text-gold-600 bg-gold-50 border border-gold-200 px-1.5 py-0.5 rounded-full">
                                                    {f.discount}
                                                </span>
                                            )}
                                            <ArrowRight className="w-3 h-3 text-slate-300 group-hover:text-gold-400 transition-colors shrink-0" />
                                        </button>
                                    ))}
                                </div>
                                ...
                                <div className="flex flex-wrap gap-2 mb-6">
                                    {PAST_PARTNER_NAMES.map((name) => (
                                        <span key={name} className="px-3.5 py-1.5 bg-slate-100 text-slate-600 text-xs font-medium rounded-full border border-slate-200">
                                            {name}
                                        </span>
                                    ))}
                                </div>
